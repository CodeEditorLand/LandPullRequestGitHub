...
import {
	findDotComAndEnterpriseRemotes,
	getEnterpriseUri,
	hasEnterpriseUri,
	setEnterpriseUri,
} from "./utils";
...
export class RepositoriesManager extends Disposable {
	...
	insertFolderManager(folderManager: FolderRepositoryManager) {
		...
		if (workspaceFolders) {
			...
			if (index > -1) {
				const arrayEnd = this._folderManagers.slice(
					index,
	...
	removeRepo(repo: Repository) {
		const existingFolderManagerIndex = this._folderManagers.findIndex(
			(manager) =>
				manager.repository.rootUri.toString() ===
	...
	getManagerForIssueModel(
		issueModel: IssueModel | undefined,
	): FolderRepositoryManager | undefined {
	...
	getManagerForFile(uri: vscode.Uri): FolderRepositoryManager | undefined {
		...
		const folderManagers = this._folderManagers
			...
			.sort(
				(a, b) =>
					b.repository.rootUri.path.length -
					a.repository.rootUri.path.length,
			);
	...
	async authenticate(enterprise?: boolean): Promise<boolean> {
		...
		// If we have no github.com remotes, but we do have github remotes, then we likely have github enterprise remotes.
			...
			!hasEnterpriseUri() &&
			dotComRemotes.length === 0 &&
			enterpriseRemotes.length > 0
		) {
			const promptResult = await vscode.window.showInformationMessage(
				vscode.l10n.t(
					"It looks like you might be using GitHub Enterprise. Would you like to set up GitHub Pull Requests and Issues to authenticate with the enterprise server {0}?",
					enterpriseRemotes[0].normalizedHost,
				),
				{ modal: true },
				yes,
				vscode.l10n.t("No, use GitHub.com"),
			);
		...
		const hasNonDotComRemote =
			enterpriseRemotes.length > 0 || unknownRemotes.length > 0;

		if (
			(hasEnterpriseUri() || dotComRemotes.length === 0) &&
			hasNonDotComRemote
		) {
			githubEnterprise = await this._credentialStore.login(
		...
		if (
			!githubEnterprise &&
			(!hasEnterpriseUri() || enterpriseRemotes.length === 0)
		) {
